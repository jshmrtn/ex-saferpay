defmodule ExSaferpay.TransactionTest do
  ...
  describe "initialize/1" do
    ...
    test "works", %{response_mocks: response_mocks} do
      with_response_mocks response_mocks do
        request =
          Request.TransactionInitialize.generate(%{
    ...
    test "gives correct error", %{response_mocks: response_mocks} do
      with_response_mocks response_mocks do
        ...
        assert {:error,
                {400,
                 %ExSaferpay.Response.ErrorResponse{
                   behavior: :abort,
                   error_detail: [
                     "ReturnUrls: The ReturnUrls field is required.",
                   ...
                   response_header: %ExSaferpay.Response.ResponseHeader{
                     request_id: "43d3b46c-8eb2-4a1f-86cf-b2ee097f7a79",
                     spec_version: "1.8"
                   },
                   transaction_id: nil
  ...
  describe "authorize_direct/1" do
    ...
    test "works with alias", %{response_mocks: response_mocks} do
      with_response_mocks response_mocks do
        request =
          Request.TransactionAuthorizeDirect.generate(%{
            payment: %Request.Payment{
              amount: Money.new(500, :USD)
            },
            payment_means: %Request.PaymentMeans{
        ...
        assert {:ok, %Response.TransactionAuthorizeDirect{}} =
                 Transaction.authorize_direct(request)
      end
    end
  end
  ...
  describe "authorize_referenced/1" do
    setup do
      ...
      with_response_mocks response_mocks do
        ...
        {:ok, %Response.TransactionAuthorizeDirect{} = authorization} =
          Transaction.authorize_direct(request)
    ...
    test "works ", %{response_mocks: response_mocks, initial_authorization: initial_authorization} do
      with_response_mocks response_mocks do
        request =
          Request.TransactionAuthorizeReferenced.generate(%{
            payment: %Request.Payment{
              amount: Money.new(500, :USD)
            },
        ...
        assert {:ok, %Response.TransactionAuthorizeReferenced{}} =
                 Transaction.authorize_referenced(request)
      end
    end
  end
  ...
  describe "capture/1" do
    ...
    test "works with alias", %{
      ...
      authorization: %{transaction: %{id: transaction_id}}
    } do
      with_response_mocks response_mocks do
        request =
          Request.TransactionCapture.generate(%{
            amount: Money.new(500, :USD),
            transaction_reference: %Request.TransactionReference{
  ...
  describe "authorize/1" do
    setup do
      ...
      with_response_mocks response_mocks do
        request =
          Request.AliasInsertDirect.generate(%{
        ...
        {:ok, %Response.AliasInsertDirect{} = alias_insert_direct} =
          SecureAliasStore.insert_direct(request)
        ...
        {:ok, %Response.TransactionInitialize{} = initialization} =
          Transaction.initialize(request)
    ...
    test "works with alias", %{response_mocks: response_mocks, initialization: %{token: token}} do
      with_response_mocks response_mocks do
        request =
          Request.TransactionAuthorize.generate(%{
            verification_code: "123",
            token: token
          })
